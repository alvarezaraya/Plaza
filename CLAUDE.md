...
**Fuentes** (12, en `FUENTES_ACTIVAS`; cada una corre en try/except — si una falla, las demás siguen): EsquinaRetornable · CulturaAntofagasta (regionales) · Ticketplus (solo página de la región) · Ticketpro · PuntoTicket · Ticketmaster · Passline · ComediaTicket · Ticketchile · MasQueTickets · Eventbrite · Joinnus (nacionales, filtradas por ciudad).

**Feed RSS municipal** (`_scrape_rss_municipal`, usado por CulturaAntofagasta): es un blog de noticias, no de eventos. `_rss_es_evento` filtra notas de prensa y recopilaciones (`RSS_RUIDO` vs `RSS_EVENTO`); la ubicación se infiere con `detectar_ciudad`/`detectar_venue` sobre el título (feed = fallback); `limpiar_nombre_rss` limpia el titular sin borrar meses ni ciudades. Las fechas sin año se anclan al `<pubDate>` del post (no se bumpean a futuro).
